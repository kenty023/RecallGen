...
// ── ACTIVITY LOG HELPER ───────────────────────────────────────────────────────
async function logActivity(user, type, description, fileName = null, meta = null) {
...
async function loadSettings(user) {
    ...
    if (data) {
        ...
        document.getElementById('acc-username')._oldValue   = data.username || '';
        document.getElementById('theme-color')._oldValue    = data.theme_color || '';
    }
}
...
document.getElementById('save-settings-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        const oldUsername   = document.getElementById('acc-username')._oldValue || '';
        const oldThemeColor = document.getElementById('theme-color')._oldValue  || '';
        const newThemeColor = document.getElementById('theme-color').value;

        // 1. Username changed
        if (newUsername && newUsername !== oldUsername) {
            changesLog.push({
                type:        'settings_username',
                description: `Changed username`,
                meta:        { old_name: oldUsername, new_name: newUsername }
            });
        }

        // 2. Profile picture changed
        if (tempPfpFile) {
            ...
            changesLog.push({
                ...
                fileName:    tempPfpFile.name
            });
        }

        // 3. Theme background changed or removed
        if (removeBg) {
            dataToSave.theme_bg = '';
            changesLog.push({
                type:        'settings_bg',
            ...
            changesLog.push({
                type:        'settings_bg',
                description: 'Changed background image'
            });
        }

        // 4. Theme color changed
        if (newThemeColor && newThemeColor.toLowerCase() !== oldThemeColor.toLowerCase()) {
            changesLog.push({
                type:        'settings_color',
                description: `Changed theme color`,
                meta:        { old_color: oldThemeColor, new_color: newThemeColor }
            });
        }
        ...
        // Update old values for next change detection
        document.getElementById('acc-username')._oldValue  = newUsername;
        document.getElementById('theme-color')._oldValue   = newThemeColor;
